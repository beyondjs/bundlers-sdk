...
module.exports = class extends DynamicProcessor {
	...
	constructor(packager) {
		...
		this.#supported = packager.application.bundlers.get(bundle.type).meta.bundle.processors;
		...
		super.setup(
			new Map([
				['bundle', { child: bundle }],
				['bundle.settings', { child: bundle.settings }],
			])
		);
	}
	...
	_process() {
		...
		for (const [name, config] of processors) {
			...
			if (this.#supported.includes(name) && !this.#packager.bundle.settings.processors.has(name)) {
